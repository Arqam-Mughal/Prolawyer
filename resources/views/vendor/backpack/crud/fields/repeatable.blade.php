...
@if ($crud->fieldTypeNotLoaded($field))
    ...
    @push('crud_fields_scripts')
        <script>
            ...
            function reLabelPetitionerRespondent() {
                var petitioners = $('.form-group[bp-field-name="petitioner_and_advocates"]');
                var rep_pelements = petitioners.find('.repeatable-element');

                rep_pelements.each(function(i, rep_pelement){
                    var row_no = $(rep_pelement).attr("data-row-number");
                    var labels = $(rep_pelement).find('label');
                    labels.each(function(j, label){
                        if ( "petitioner" === $(label).parent().attr("bp-field-name") ) {
                            if ( rep_pelements.length === 1 ){
                                $(label).text( "Petitioner" );
                            } else {
                                $(label).text( "Petitioner "+row_no );
                            }
                        } else {
                            if ( rep_pelements.length === 1 ){
                                $(label).text( "Petitioner's Advocate" );
                            } else {
                                $(label).text( "Petitioner's Advocate "+row_no );
                            }
                        }
                    });
                });
                
                var respondents = $('.form-group[bp-field-name="respondent_and_advocates"]');
                var rep_relements = respondents.find('.repeatable-element');
                
                rep_relements.each(function(i, rep_relement){
                    var row_no = $(rep_relement).attr("data-row-number");
                    var labels = $(rep_relement).find('label');
                    labels.each(function(j, label){
                        if ( "respondent" === $(label).parent().attr("bp-field-name") ) {
                            if ( rep_relements.length === 1 ){
                                $(label).text( "Respondent" );
                            } else {
                                $(label).text( "Respondent "+row_no );
                            } 
                        } else {
                            if ( rep_relements.length === 1 ){
                                $(label).text( "Respondent's Advocate" );
                            } else {
                                $(label).text( "Respondent's Advocate "+row_no );
                            } 
                        }
                    });
                });
