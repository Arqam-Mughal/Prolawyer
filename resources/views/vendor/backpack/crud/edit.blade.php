...
@push('after_scripts')
    ...
    <script>
        ...
        var user = {{ backpack_user()->hasRole('Super admin') && \Illuminate\Support\Facades\Route::is('user.edit') ? 'true' : 'false' }};
// alert(user);
if (user == true) {
    var userPermissions = @json(
        \App\Models\User::where('id', request()->route('id'))->first()
        ? \App\Models\User::where('id', request()->route('id'))->first()->getAllPermissions()->pluck('id')
        : []
    );
} else {
    var userPermissions = [];
...
var seniorLawyerId = <?php echo json_encode($crud->fields()['senior_lawyer_id']['value'] ?? null); ?>;
        ...
        checklistHtml += `
            <button class="btn btn-primary dropdown-toggle w-100 hskdhksd"
                ...
                <div class="checkbox checkbox-main">
                    <input name="permissions[]" id="mainitem${permission.id}" type="checkbox" value="${permission.id}" class="main-checkbox" ${userPermissions.includes(permission.id) ? 'checked' : ''}>
            ...
            checklistHtml += `
                <li class="dropdown-item dropdown">
                    <label class="font-weight-normal w-100" for="subitem${subitem.id}">
                        <input name="permissions[]" id="subitem${subitem.id}" type="checkbox" value="${subitem.id}" ${userPermissions.includes(subitem.id) ? 'checked' : ''}>
                        ${subitem.name}
                    </label>`;
                    ...
                    checklistHtml += `
                        <li class="dropdown-item">
                            <label class="font-weight-normal w-100" for="sub_subitem${sub_subitem.id}">
                                <input id="sub_subitem${sub_subitem.id}" name="permissions[]" type="checkbox" value="${sub_subitem.id}" ${userPermissions.includes(sub_subitem.id) ? 'checked' : ''}>
                                ${sub_subitem.name}
                            </label>
                        </li>`;
                });
                            ...
                            checklistHtml += `
                                <div class="bg-primary text-white justify-content-center text-center h-100 py-2 d-flex align-items-center" style="border-radius: 4px;">
                                    <label class="font-weight-normal w-100" for="subitem${permission.id}" style=" font-weight: bold; ">
                                        <input id="subitem${permission.id}" name="permissions[]" type="checkbox" value="${permission.id}" ${userPermissions.includes(permission.id) ? 'checked' : ''}>
                                        ${permission.name}
                                    </label>
                                </div>`;
                        }
                        ...
                        if (subitems.length) {
                            checklistHtml += `
                                <button class="btn btn-primary dropdown-toggle w-100 hskdhksd"
                                        ...
                                        data-bs-toggle="dropdown" aria-expanded="false">
                                    <div class="checkbox checkbox-main">
                                        <input name="permissions[]" id="mainitem${permission.id}" class="main-checkbox" type="checkbox" value="${permission.id}">
                                ...
                                if (sub_subitems.length) {
                                    ...
                                    $.each(sub_subitems, function (sub_subindex, sub_subitem) {
                                        checklistHtml += `
                                            <li class="dropdown-item">
                                                <label class="font-weight-normal w-100" for="sub_subitem${sub_subitem.id}">
                                                    <input id="sub_subitem${sub_subitem.id}" name="permissions[]" type="checkbox" value="${sub_subitem.id}">
                                                    ${sub_subitem.name}
                                                </label>
                                            </li>`;
                                    });
                            ...
                            checklistHtml += `
                                <div class="bg-primary text-white justify-content-center text-center h-100 py-2 d-flex align-items-center" style="border-radius: 4px;">
                                    <label class="font-weight-normal w-100" for="subitem${permission.id}" style=" font-weight: bold; ">
                                        <input id="subitem${permission.id}" name="permissions[]" type="checkbox" value="${permission.id}">
                                        ${permission.name}
                                    </label>
                                </div>`;
                        }
